...
class Command(BaseCommand):
    help = 'Seeds the DB with the absolute Premium UI JSON structure and Categories'

    def handle(self, *args, **options):
        self.stdout.write(self.style.WARNING("Wiping existing Providers & Categories via Raw SQL..."))
        
        with connection.cursor() as cursor:
            cursor.execute("PRAGMA foreign_keys = OFF;")
            # Clear marketplace-linked deals to avoid constraint errors
            cursor.execute("DELETE FROM compras_proveedoroferta;") 
        ...
        self.stdout.write("Creating Exact Premium Category Taxonomy...")

        # Root Categories
        lab_cat = Categoria.objects.create(nombre="Laboratorio", orden=1)
        fin_cat = Categoria.objects.create(nombre="Financieras", orden=2)
        imp_cat = Categoria.objects.create(nombre="Implantología", orden=3)
        ort_cat = Categoria.objects.create(nombre="Ortodoncia", orden=4)
        dep_cat = Categoria.objects.create(nombre="Depósitos y Aparatología", orden=5) # User missed this but 13 providers belong here
        srv_cat = Categoria.objects.create(nombre="Servicios", orden=6)

        # Subcategories for 'Servicios'
        sub_gestion = Categoria.objects.create(nombre="Gestión y marketing dental", parent=srv_cat, orden=1)
        sub_soft = Categoria.objects.create(nombre="Software y Ciberseguridad", parent=srv_cat, orden=2)
        sub_radio = Categoria.objects.create(nombre="Radiología UTPR y Dosimetría", parent=srv_cat, orden=3)
        sub_gest = Categoria.objects.create(nombre="Gestoría y Seguros", parent=srv_cat, orden=4)
        sub_otros = Categoria.objects.create(nombre="Otros", parent=srv_cat, orden=5)

        # Mapping dictionary
        cat_map = {
            "Depósitos, Aparatología y SAT": dep_cat,
            "Laboratorios": lab_cat,
            "Gestión, Marketing y Selección de Personal": sub_gestion,
            "Implantología, biomaterial y anestesia": imp_cat,
            "Ortodoncia": ort_cat,
            "Ciberseguridad, Telco y Software": sub_soft,
            "Radiología, UTPR y Dosimetría": sub_radio,
            "Financiación de tratamientos": fin_cat,
            "Gestoría, Seguros, y Legal": sub_gest,
            "Desinfección, Higiene, Análisis y Farma": sub_otros,
            "Interiorismo, Mat. Oficina y Otros": sub_otros
        }
        ...
        self.stdout.write("Planting Providers...")
        for prov_data in json_data['proveedores']:
            prov_cat = cat_map.get(prov_data['categoria'], sub_otros)
            ...
            p = Proveedor.objects.create(
                ...
                url_web=f"https://{prov_data['email'].split('@')[-1]}" if '@' in prov_data['email'] else "",
                tipo_interaccion='link',
            )
            p.categorias.add(prov_cat)
            
        self.stdout.write(self.style.SUCCESS(f"✅ Success! Generated all {len(json_data['proveedores'])} premium providers."))
